...
const NavigationItem = ({ item, isAcitve, onClick, isCollapsed }) => {
    ...
    >
      <Icon
        className={`h-5 w-5 flex-shrink-0 ${
          isAcitve ? "text-blue-900" : "text-gray-500"
        }`}
      />
...
const DashboardLayout = ({ children, activeMenu }) => {
  ...
  const [activeNavItem, setAciveNavItem] = useState(activeMenu || "dashboard");
  ...
  return (
    <div className="flex h-screen bg-gray-50">
        ...
        <nav className="p-4 space-y-2">
          {NAVIGATION_MENU.map((item) => (
            <NavigationItem
              key={item.id}
              item={item}
              isAcitve={activeNavItem === item.id}
      ...
      {/* Main content */}
      <div
        className={`flex-1 flex flex-col trasnition-all duration-300 ${
